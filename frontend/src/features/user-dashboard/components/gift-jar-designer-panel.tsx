...
export default function GiftJarDesignerPanel({
  ...
  useEffect(() => {
    ...
    setLocalDanceScale(settings.jarDanceScale !== undefined ? settings.jarDanceScale : 1.0);
    setLocalDanceOffsetX(settings.jarDanceOffsetX !== undefined ? settings.jarDanceOffsetX : 185);
    
    const col = settings.jarColor || '#ffffff';
    setLocalColorInput(col);
    const { alpha } = parseColorToHexAndAlpha(col);
    setLocalAlpha(alpha);
  }, [settings]);
  ...
  const [simulatedGifts, setSimulatedGifts] = useState<{ id: number; icon: string }[]>([
    { id: 1, icon: 'https://sf16-website-nos.sofproxy.com/obj/tiktok-web-tx/tiktok/web/gift/rose.png' },
    { id: 2, icon: 'https://sf16-website-nos.sofproxy.com/obj/tiktok-web-tx/tiktok/web/gift/rose.png' },
    { id: 3, icon: 'https://sf16-website-nos.sofproxy.com/obj/tiktok-web-tx/tiktok/web/gift/rose.png' },
  ]);
  ...
  const handleSimulateDrop = (count: number) => {
    const newItems = Array.from({ length: count }, (_, i) => ({
      id: Date.now() + i,
      icon: 'https://sf16-website-nos.sofproxy.com/obj/tiktok-web-tx/tiktok/web/gift/rose.png',
    }));
    setSimulatedGifts((prev) => [...prev.slice(-15), ...newItems]);
            ...
            <div className="flex flex-col gap-3 pt-3 border-t border-border-color/20 animate-[fade-in-up_0.2s_ease-out]">
              <div className="flex justify-between items-center select-none">
                ...
                <label className="relative inline-flex items-center cursor-pointer select-none">
                  <input 
                    type="checkbox" 
                    checked={settings.jarDanceEnabled !== false} 
